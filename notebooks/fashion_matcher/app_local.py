...
from models.image_processor_local import ImageProcessor
...
if __name__ == "__main__":
    try:
        ...
        print('Starting StyleFinderApp initialization...')
        app = StyleFinderApp("swift-style-embeddings.pkl")
        print('StyleFinderApp initialized')

        # Create the Gradio interface
        print('Creating Gradio interface...')
        demo = create_gradio_interface(app)
        print('Gradio interface created:', type(demo))

        # Launch the Gradio interface
        env_port = os.environ.get("GRADIO_SERVER_PORT")
        port = int(env_port) if env_port else None
        print(f'Launching Gradio at 127.0.0.1:{port or "(auto)"} (share=True)')
        demo.launch(
