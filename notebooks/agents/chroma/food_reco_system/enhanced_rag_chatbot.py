...
'''This section connects to IBM watsonx.ai to access the Granite large language model,
 which will be used to generate intelligent, context-aware responses. 
 The model processes both user queries and relevant food data to 
...
'''
This function transforms the raw search results into a structured, readable format
that the LLM can understand and use effectively. It extracts key information like
...
'''
 This function implements the core RAG workflow by combining information 
 retrieval with language generation. It constructs an augmented prompt that 
 includes both the user's original prompt and the retrieved contextual data. 
 This prompt is then sent to IBM watsonx.ai, which provides access to the 
 IBM Granite LLM. IBM Granite uses this combined input to generate intelligent, 
 context-aware responses that are natural and helpful.
 '''
...
'''
This fallback function ensures the chatbot always provides a response, even if 
the LLM connection fails. It uses the search results to generate a basic but 
helpful recommendation, maintaining system reliability.
'''
...
The main chatbot interface provides a conversational experience where 
users can ask natural language questions about food. It maintains conversation 
history for context and handles various commands while focusing on the core 
RAG functionality.
'''
...
'''
 This function orchestrates the complete RAG process: performing vector search, 
 generating AI responses, and displaying results. It provides transparency by 
 showing both the AI-generated response and the underlying search results that 
 informed it.
 '''
...
'''
The enhanced comparison mode uses the LLM to provide intelligent analysis 
of the differences and similarities between two food queries, offering insights 
that go beyond simple result matching.
'''
