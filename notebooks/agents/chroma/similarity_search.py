...
def perform_similarity_search(collection, all_items, query_term=["apple"]):
    try:
...
def main():
    try:
        ...
        # metadatas are optional metadata associated with each document, in this case, we are 
        # adding a source and category for each document
        collection.add(
            documents=texts,
        ...
        perform_similarity_search(collection, all_items, ["red", "fresh"])
